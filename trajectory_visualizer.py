...
def draw_sample(
    ...
    fixed_window: Tuple[float, float, float, float],
    debug: bool,
) -> bool:
    token = item["token"]
    metric = item["metric"]
    ...
    l2_avg_plot = calc_l2_3s(pred_best, gt)
    col_3s = metric.get("plan_obj_box_col_3s", None)
    title = f"{token[:8]}"
    if l2_3s is not None:
        title += f" | L2_3s={float(l2_3s):.3f}"
    if np.isfinite(l2_avg_plot):
        title += f" | L2_avg={l2_avg_plot:.3f}"
    if col_3s is not None:
        title += f" | col_3s={float(col_3s):.4f}"
    ax.set_title(title, fontsize=12)

    xmin, xmax, ymin, ymax = fixed_window
    ax.set_xlim(xmin, xmax)
    ax.set_ylim(ymin, ymax)
    ax.set_aspect("equal")
    ax.grid(True, linestyle="--", linewidth=0.6, alpha=0.45)
    ax.axhline(0.0, color="gray", linewidth=1.0, alpha=0.8, zorder=0)
    ax.axvline(0.0, color="gray", linewidth=1.0, alpha=0.8, zorder=0)
    ax.set_xlabel("x (m)")
    ax.set_ylabel("y (m)")
...
def main() -> None:
    ...
    parser.add_argument(
        "--fixed-window",
        default="-10,10,-10,10",
        help="Fixed plot window as xmin,xmax,ymin,ymax (same for every image)",
    )
    ...
    try:
        win = [float(x.strip()) for x in args.fixed_window.split(",")]
        if len(win) != 4 or win[0] >= win[1] or win[2] >= win[3]:
            raise ValueError
        fixed_window = (win[0], win[1], win[2], win[3])
    except Exception:
        raise ValueError("--fixed-window must be xmin,xmax,ymin,ymax with min<max")
